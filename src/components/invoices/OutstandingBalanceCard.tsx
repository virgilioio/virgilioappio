...
export function OutstandingBalanceCard({ invoices }: OutstandingBalanceCardProps) {
  ...
  const totalOutstanding = useMemo(() => {
    ...
    // Filter outstanding invoices (pending + overdue) by date range
    const filteredInvoices = invoices.filter(invoice => {
      const invoiceDate = new Date(invoice.issued_at)
      const isOutstanding = invoice.status === 'pending' || invoice.status === 'overdue'
      return isOutstanding && invoiceDate >= startDate && invoiceDate <= now
    })

    // Calculate total amount of outstanding invoices in the selected period
