...
export function GioPointsInbox({
  ...
  // Treat legacy "validated"/"flagged" as "added" so prior work surfaces correctly.
  ...
  return (
    <Collapsible
      ...
      className="rounded-xl border border-[#E7E8EE] bg-white"
    >
      <CollapsibleTrigger className="w-full flex items-start justify-between gap-3 p-4 text-left">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-[15px] font-semibold tracking-[-0.02em] text-[#1F2230] font-poppins">
              Points to validate
            </span>
          </div>
          <p className="text-[12.5px] text-[#5A6072] leading-snug">
            Gio flagged these from the résumé and job description. Add the useful ones to your
            scorecard, or dismiss them.
          </p>
        </div>
      ...
      <CollapsibleContent>
        <div className="border-t border-[#F1F0EC] p-3 space-y-2">
          {allReviewed ? (
            <div className="flex items-center gap-2 px-2 py-3 text-[12.5px] text-[#5A6072]">
            ...
            pending.map(({ point, index }) => (
              <div
                key={index}
                className="flex items-start gap-3 rounded-lg border border-[#F1F0EC] bg-[#FAFAF7] p-3"
              >
