...
export function ApplicationFieldForm({ field, onClose, onSaved }: { field?: ApplicationField | null; onClose: () => void; onSaved: () => Promise<void> | void }) {
  ...
  return (
    <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>{field ? 'Edit Application Field' : 'Add Application Field'}</DialogTitle>
        <DialogDescription>
          {field ? 'Update application field configuration' : 'Define a new field for job application forms'}
        </DialogDescription>
      </DialogHeader>
