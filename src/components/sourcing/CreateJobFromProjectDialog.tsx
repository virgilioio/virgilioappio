...
export function CreateJobFromProjectDialog({
  ...
  const orgOptions = (childOrgs || []).map(org => ({
    value: org.id,
    label: org.name
  }))
  ...
  const handleCreateJob = async () => {
    if (!selectedOrgId) {
      toast({
        ...
        description: 'Please select a Job Folder where the job will be created.',
        variant: 'destructive'
      })
      return
    }
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <SearchableSelect
              ...
              placeholder={isLoadingOrgs ? "Loading folders..." : "Select a Job Folder"}
              disabled={isLoadingOrgs}
            />
            <p className="text-xs text-muted-foreground">
              The job will be created in this folder and start as a draft.
            </p>
          </div>
        </div>
