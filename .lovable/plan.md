...
- Emit server-timestamped `phase` events around the real snapshot read, provider wait, and streamed write. Emit `stats` immediately after the database read and `issues` immediately after local detector evaluation. Defer displaying narrated phases until the cache decision is known, so cache hits never replay work.
- Omit the `snapshot` phase: the current briefing path does not perform a genuine redaction pass, so the UI must not claim that it does. Preserve the existing model payload and briefing behavior.
...
- Do not claim comparable-role counts because the current system has no comparable-role query. Receipt/detail copy will include only measured candidate, stage, activity-window, and phase-duration values.
- Propagate explicit user cancellation to the provider request and emit useful provider, parse, persistence, and interrupted-stream errors without discarding already emitted local data or prose. The 10-second warning is UI-only; generation is never aborted by a timer. If the provider itself fails or closes around 30 seconds, surface that real failure rather than synthesizing one.
