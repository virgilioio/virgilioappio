...
-- Add unique constraint to invite_token for security
ALTER TABLE public.members 
ADD CONSTRAINT unique_invite_token UNIQUE (invite_token);

-- Add index for better performance on token lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_members_invite_expires_at 
ON public.members(invite_expires_at) 
WHERE user_status = 'invited';

-- Improve the validate_invite_token function with better security
...
-- Enhanced accept_invitation function with better error handling
