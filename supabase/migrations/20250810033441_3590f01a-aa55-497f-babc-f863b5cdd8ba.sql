-- Normalize check constraints to support application_field_id as a valid owner, ensuring exactly one FK is set
ALTER TABLE public.field_select_options 
DROP CONSTRAINT IF EXISTS field_select_options_single_reference;
ALTER TABLE public.field_validation_rules 
DROP CONSTRAINT IF EXISTS field_validation_rules_single_reference;
ALTER TABLE public.field_select_options 
DROP CONSTRAINT IF EXISTS field_select_options_reference_check;

-- Recreate constraints with 3-way exclusivity
ALTER TABLE public.field_select_options 
