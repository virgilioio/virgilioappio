-- Create application limits tracking table
CREATE TABLE public.candidate_application_limits (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  candidate_email TEXT NOT NULL,
  job_id UUID NOT NULL,
  posting_id UUID,
  organization_id UUID NOT NULL,
  applied_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  status TEXT NOT NULL DEFAULT 'active', -- 'active', 'rejected', 'withdrawn', 'offered'
  status_updated_at TIMESTAMP WITH TIME ZONE,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create indexes for efficient querying
CREATE INDEX idx_candidate_application_limits_email ON public.candidate_application_limits(candidate_email);
CREATE INDEX idx_candidate_application_limits_email_org ON public.candidate_application_limits(candidate_email, organization_id);
CREATE INDEX idx_candidate_application_limits_email_job ON public.candidate_application_limits(candidate_email, job_id);
CREATE INDEX idx_candidate_application_limits_applied_at ON public.candidate_application_limits(applied_at);

-- Enable RLS
ALTER TABLE public.candidate_application_limits ENABLE ROW LEVEL SECURITY;

-- Create policies for application limits
CREATE POLICY "Organization members can view application limits" 
ON public.candidate_application_limits 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 FROM public.members m 
    WHERE m.user_id = auth.uid() 
    AND m.organization_id = candidate_application_limits.organization_id 
    AND m.user_status = 'active'
  ) 
  OR get_user_type_secure() = 'platform_admin'
);

CREATE POLICY "Organization recruiters can manage application limits" 
ON public.candidate_application_limits 
FOR ALL 
USING (
  EXISTS (
    SELECT 1 FROM public.members m 
    WHERE m.user_id = auth.uid() 
    AND m.organization_id = candidate_application_limits.organization_id 
    AND m.member_role IN ('admin', 'recruiter') 
    AND m.user_status = 'active'
  ) 
  OR get_user_type_secure() = 'platform_admin'
);

-- Create function to check application limits
CREATE OR REPLACE FUNCTION public.check_application_limits(
  candidate_email_param TEXT,
  job_id_param UUID,
  organization_id_param UUID
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
DECLARE
  total_applications_60_days INTEGER;
  same_job_last_application TIMESTAMP WITH TIME ZONE;
  last_rejected_application TIMESTAMP WITH TIME ZONE;
  result JSONB;
BEGIN
  -- Check total applications in last 60 days across all jobs
  SELECT COUNT(*) INTO total_applications_60_days
  FROM public.candidate_application_limits
  WHERE candidate_email = candidate_email_param
    AND organization_id = organization_id_param
    AND applied_at >= (now() - INTERVAL '60 days');
  
  -- Check last application to same job
  SELECT MAX(applied_at) INTO same_job_last_application
  FROM public.candidate_application_limits
  WHERE candidate_email = candidate_email_param
    AND job_id = job_id_param;
  
  -- Check last rejected application (any job in org)
  SELECT MAX(status_updated_at) INTO last_rejected_application
  FROM public.candidate_application_limits
  WHERE candidate_email = candidate_email_param
    AND organization_id = organization_id_param
    AND status = 'rejected';
  
  -- Build result object
  result := jsonb_build_object(
    'can_apply', true,
    'total_applications_60_days', total_applications_60_days,
    'max_applications_60_days', 3,
    'violations', jsonb_build_array()
  );
  
  -- Check limit violations
  IF total_applications_60_days >= 3 THEN
    result := jsonb_set(result, '{can_apply}', 'false');
    result := jsonb_set(result, '{violations}', 
      (result->'violations') || jsonb_build_object(
        'type', 'max_applications_exceeded',
        'message', 'You have reached the maximum of 3 applications in the last 60 days'
      )
    );
  END IF;
  
  -- Check same job reapplication cooldown (90 days)
  IF same_job_last_application IS NOT NULL AND same_job_last_application > (now() - INTERVAL '90 days') THEN
    result := jsonb_set(result, '{can_apply}', 'false');
    result := jsonb_set(result, '{violations}', 
      (result->'violations') || jsonb_build_object(
        'type', 'same_job_cooldown',
        'message', 'You cannot reapply to the same job within 90 days',
        'cooldown_until', (same_job_last_application + INTERVAL '90 days')
      )
    );
  END IF;
  
  -- Check rejection cooldown (30 days)
  IF last_rejected_application IS NOT NULL AND last_rejected_application > (now() - INTERVAL '30 days') THEN
    result := jsonb_set(result, '{can_apply}', 'false');
    result := jsonb_set(result, '{violations}', 
      (result->'violations') || jsonb_build_object(
        'type', 'rejection_cooldown',
        'message', 'You cannot apply within 30 days of a rejection',
        'cooldown_until', (last_rejected_application + INTERVAL '30 days')
      )
    );
  END IF;
  
  RETURN result;
END;
$$;