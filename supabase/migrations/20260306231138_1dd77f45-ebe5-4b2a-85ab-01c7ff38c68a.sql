...
-- Add missing state alias and re-run
...
BEGIN
  ...
  IF LOWER(COALESCE(NEW.location_country, '')) IN ('país', 'null', 'n/a', '-', '--', 'none') THEN
  ...
  -- Fix cities wrongly stored in country field
  ...
  -- Fix "State, Mexico" in country field
  ...
  -- State: strip city from composite "City, State" values
  ...
  IF NEW.location_state IS NOT NULL THEN
    ...
    NEW.location_state := CASE
      ...
      WHEN lower_state IN ('cdmx', 'ciudad de méxico', 'ciudad de mexico', 'federal district', 'distrito federal', 'mexico city', 'mexico city metropolitan') THEN 'Mexico City'
  ...
  IF NEW.location_city IS NOT NULL THEN
    ...
    NEW.location_city := CASE TRIM(LOWER(NEW.location_city))
      WHEN 'ciudad de méxico' THEN 'Mexico City' WHEN 'ciudad de mexico' THEN 'Mexico City'
      WHEN 'cdmx' THEN 'Mexico City' WHEN 'méxico city' THEN 'Mexico City'
      WHEN 'mexico city' THEN 'Mexico City' WHEN 'df' THEN 'Mexico City'
      WHEN 'santiago de querétaro' THEN 'Querétaro' WHEN 'santiago de queretaro' THEN 'Querétaro'
      WHEN 'queretaro' THEN 'Querétaro' WHEN 'querétaro' THEN 'Querétaro'
      WHEN 'sao paulo' THEN 'São Paulo' WHEN 'são paulo' THEN 'São Paulo'
      WHEN 'bogota' THEN 'Bogotá' WHEN 'bogotá' THEN 'Bogotá'
      WHEN 'medellin' THEN 'Medellín' WHEN 'medellín' THEN 'Medellín'
      WHEN 'cancun' THEN 'Cancún' WHEN 'cancún' THEN 'Cancún'
      WHEN 'merida' THEN 'Mérida' WHEN 'mérida' THEN 'Mérida'
      WHEN 'leon' THEN 'León' WHEN 'león' THEN 'León'
      WHEN 'monterrey' THEN 'Monterrey' WHEN 'guadalajara' THEN 'Guadalajara'
      WHEN 'puebla' THEN 'Puebla' WHEN 'tijuana' THEN 'Tijuana'
...
-- Re-trigger on remaining stragglers
